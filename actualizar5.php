<html>
	<head>
		<title>Modificar confirmacion</title>
		<meta charset="UTF-8" />
		<link rel="stylesheet" href="stylesactualizar5.css">
	</head>
	<body>
	<nav class="navbar">
		...
		<?php
			...
			$sumarlibros=$conexionlibros -> query ("SELECT Cantidad from libros WHERE Titulo= '{$_POST['CantidadNueva']}'") or die ("Ha fallado la conexión");
			$conexion = null;
			$consulta = $conexionlibros -> query ("UPDATE libros SET Cantidad = '{$_POST['CantidadNueva']}' WHERE Titulo= '{$_POST['Cantidadlibros']}' ") or die ("Ha fallado la conexión");
			$conexion = null;

			echo '<p>Cantidad Cambiada</p>';
		?>
		<footer class="footer">
        <div class="waves">
