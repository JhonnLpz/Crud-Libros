<html>
	<head>
		...
		<meta charset="UTF-8" />
		<link rel="stylesheet" href="stylesmodificar.css">
	</head>
	<body>
	<nav class="navbar">
		...
		<h1 class="h1"><strong>Cambiar Cantidad de libros</strong></h1>
		<div class="formulario">

		<form action="actualizar5.php" method="post">
			<select name="Cantidadlibros">
			<?php
				include 'conexionlibros.php';
				$consulta = $conexionlibros -> query ("SELECT Cantidad,Titulo FROM libros ORDER BY Titulo ASC") or die ("Ha fallado la conexión");
					while ( $registro = $consulta -> fetch_assoc()) {
						echo'<option>'.$registro['Titulo'].'</option>';
					}
			?>
			</select>
			<input type="Number" min="1" name="CantidadNueva" placeholder="Inserte la cantidad a modificar" required/><br/>
			<button id="boton" type="submit">Cambiar Cantidad de libros</button>
